...
export default function EntityDetailPage() {
  ...
  const rows = useMemo(() => {
    const result: (typeof queueDots)[] = [];
    for (let i = 0; i < queueDots.length; i += DOTS_PER_ROW) {
      result.push(queueDots.slice(i, i + DOTS_PER_ROW));
    }
    return result;
  }, [queueDots]);
      ...
      <div className="flex-1 px-4 py-8">
        {peopleWaiting === 0 ? (
          <div className="flex h-full items-center justify-center">
            <div className="rounded-xl border border-dashed border-zinc-300 px-6 py-4 dark:border-zinc-600">
              <span className="text-sm text-zinc-400">Sin espera</span>
            </div>
          </div>
        ) : (
          <div className="flex flex-col gap-0">
            <div className="flex items-center gap-2">
              <div className="flex h-7 w-7 items-center justify-center rounded-full border border-dashed border-zinc-300 bg-white dark:border-zinc-600 dark:bg-zinc-900">
                <svg
            ...
            {rows.map((row, rowIndex) => {
              ...
              return (
                <div key={rowIndex} className="flex flex-col gap-0">
                  <div className="flex h-3 items-center justify-center">
                    <div className="h-full w-px bg-zinc-200 dark:bg-zinc-700" />
                  </div>

                  <div
                    className={`flex items-center gap-2 ${isLTR ? "" : "flex-row-reverse"}`}
                  >
                    <div className={`h-px flex-1 bg-zinc-200 dark:bg-zinc-700 ${isLTR ? "-ml-2" : "-mr-2"}`} />
                    ...
                    <div className={`h-px flex-1 bg-zinc-200 dark:bg-zinc-700 ${isLTR ? "-mr-2" : "-ml-2"}`} />

                    <div className="flex w-5 flex-shrink-0 flex-col items-center gap-0">
            ...
            <div className="flex items-center justify-center gap-4 pt-1">
              <div className="flex flex-col items-center gap-1">
                <div
                  className={`flex h-10 w-10 items-center justify-center rounded-lg border-2 bg-white dark:bg-zinc-800 ${
                    servedDots.length >= 1
                ...
                >
                  {servedDots.length >= 1 ? (
                    <div className="h-3.5 w-3.5 rounded-full bg-emerald-500 ring-2 ring-emerald-200 dark:ring-emerald-800" />
                  ) : (
